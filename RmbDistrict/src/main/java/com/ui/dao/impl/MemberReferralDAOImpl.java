...
public class MemberReferralDAOImpl implements MemberReferralDAO {
	...
	public List<MemberOneToOne> getMemberMeetingsById(int memberid, String fromdate, String todate) {
		...
		/* and mm.member_id = o.invited_by_member_id */
		try {
			...
			while (rs.next()) {
				...
				memberOneToOne.setOneToOneId(rs.getInt("one_to_one_id"));
				memberOneToOne.setMetMemberId(rs.getInt("met_member_id"));
				memberOneToOne.setMemberFirstName(rs.getString("member_first_name"));
				memberOneToOne.setMemberMiddleName(rs.getString("member_middle_name"));
				memberOneToOne.setMemberLastName(rs.getString("member_last_name"));
				memberOneToOne.setInvitedByMemberId(rs.getInt("invited_by_member_id"));
				memberOneToOne.setInviteeFirstName(rs.getString("invitee_first_name"));
				memberOneToOne.setInviteeMiddleName(rs.getString("invitee_middle_name"));
				memberOneToOne.setInviteeLastName(rs.getString("invitee_last_name"));
				memberOneToOne.setMeetingDate(rs.getString("meeting_date"));
				memberOneToOne.setLocation(rs.getString("location"));
				memberOneToOne.setConversationTopic(rs.getString("conversation_topic"));
	...
	public List<MemberThankYouSlip> getMemberBusinessReceivedById(int memberid, String fromdate, String todate) {
		...
		List<MemberThankYouSlip> MemberThankYouSlip = new ArrayList<MemberThankYouSlip>();
		String s = "y";
		String sql = "select t.thank_you_slip_id, t.to_member_id, m.member_first_name, m.member_middle_name, m.member_last_name, t.amount, DATE_FORMAT(t.slip_date, '%d/%m/%Y') as slip_date, t.business_type, t.referral_type, t.comments from member_thank_you_slip t, members m where t.status=? and t.to_member_id=? and t.slip_date between STR_TO_DATE(?,'%d/%m/%Y') and STR_TO_DATE(?,'%d/%m/%Y') and m.member_id = t.to_member_id order by t.slip_date";
		Connection conn = null;
		try {
			conn = dataSource.getConnection();
			PreparedStatement ps = conn.prepareStatement(sql);
	...
	public void saveReferral(MemberReferral mr) {
		...
		try {
			...
			System.out.println("EEEEEEEEEEEEEEEEEEEEEEEEEEE"+e1);
			...
			if (conn != null) {
				try {
					...
					System.out.println("EEEE22222222222222222222222222EEEEEEEEEEEEEEEEEEEEEEE"+e1);
				}
			}
		}
	}
	...
	public void CloseRef(MemberReferral mr) {
		logger.info("+++++ CLOSE REFERENCE +++++");
	      String sql = "update member_referral set member_referral_status=?, comments=?, close_reasson=? where member_referral_id=?";
	      ...
	      try {
	          ...
	          System.out.println("---mr.getCloseComment()-----"+mr.getCloseComment());
	 ...
	 public List<MemberOneToOne> getMemberMeetingList(String fromdate, String todate) {
		  logger.info("+++++ GET Location NAME +++++");
		    List<MemberOneToOne> mmp = new ArrayList<MemberOneToOne>();
		    String s = "y";
		    String sql = "SELECT mo.met_member_id,mo.invited_by_member_id,mo.meeting_date as meetingDate ,mo.conversation_topic as conversationTopic,mm.member_first_name as metfirstname,mm.member_last_name as metlastname,mm.member_middle_name as metmiddlename,invite.member_first_name as invitefirstname,invite.member_last_name as invitelastname,invite.member_middle_name as invitemiddlename FROM member_one_to_one mo left join members mm on mo.met_member_id = mm.member_id left join members invite on mo.invited_by_member_id = invite.member_id where mo.meeting_date between STR_TO_DATE(?,'%d/%m/%Y') and STR_TO_DATE(?,'%d/%m/%Y') and mo.status=?";
		    Connection conn = null;
		    try {
		        conn = dataSource.getConnection();
		        PreparedStatement ps = conn.prepareStatement(sql);
		        ...
		        ResultSet rs = ps.executeQuery();
		        while (rs.next()) {
		        	MemberOneToOne m =  new MemberOneToOne();
		        	
		        	
		        	//  m.setProduct_id(rs.getInt("product_id"));  
		        	
		              m.setConversationTopic(rs.getString("conversationTopic")); 	
		              m.setMeetingDate(rs.getString("meetingDate")); 
		              m.setInviteeFirstName(rs.getString("invitefirstname")); 
		              m.setInviteeMiddleName(rs.getString("invitemiddlename")); 
		              m.setInviteeLastName(rs.getString("invitelastname")); 
		              m.setMemberFirstName(rs.getString("metfirstname"));
		              m.setMemberMiddleName(rs.getString("metmiddlename"));
		              m.setMemberLastName(rs.getString("metlastname"));
