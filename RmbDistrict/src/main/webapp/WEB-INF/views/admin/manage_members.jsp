											...
											<div class="table-responsive">
												<table class="table table-bordered">
													<tbody >
														...
														<tr ng-repeat="item in getmember1  | filter: { fellowship_id: fellowship_id}" ng-if="item.role_id != 1" style="cursor:pointer;cursor:hand">
															<td style="color:black;" ng-click='redirectmemberdetail(item.memberId)'>{{$index + 1}}</td>
															<td style="color:black;white-space: nowrap;" ng-click='redirectmemberdetail(item.memberId)'>{{item.fellowship_name}} <br> {{item.clubName}}  {{item.role_id}}</td>
